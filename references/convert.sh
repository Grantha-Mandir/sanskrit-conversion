for file in conversion_todo/*.docx
 do
	...
	mkdir unzipped
	cd unzipped
	unzip ../$file 	# this fails if the path contains a space!
	...
	sed 's/Balaram/Arial/g' < word/styles.xml > word/_styles.xml
	mv word/_styles.xml word/styles.xml

	sed 's/Balaram/Arial/g' < word/fontTable.xml > word/_fontTable.xml
	mv word/_fontTable.xml word/fontTable.xml
	zip -r ../$file *	# patch source docx file
	cd ..
	rm -rf unzipped/
 done
